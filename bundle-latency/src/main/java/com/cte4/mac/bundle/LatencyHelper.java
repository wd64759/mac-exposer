...
public class LatencyHelper extends Helper {

    protected LatencyHelper(Rule rule) {
        super(rule);
    }
    ...
    public void enableLatency(MeterRegistry registry) {
        ...
        try {
            Timer timer = Timer.builder("customer_service").tags("severity", "LOW", "function", "customer")
                    .publishPercentileHistogram().minimumExpectedValue(Duration.ofMillis(1))
                    .maximumExpectedValue(Duration.ofSeconds(5)).publishPercentiles(0.5, 0.95).register(registry);
